class LoginController < ApplicationController
  ...
  private
    ...
    def handle_post_success
      ...
      cookies[:tracks_login] = { :value => @user.login, :expires => Time.now + 1.year, :secure => Settings.secure_cookies }
      unless should_expire_sessions?
        @user.remember_me
        cookies[:auth_token] = { :value => @user.remember_token , :expires => @user.remember_token_expires_at, :secure => Settings.secure_cookies }
      end
